...
## GraphQL JSON フォーマット対応一覧

- 単純フィールド
    - スカラーの割当（文字列・数値など）
    - graphql タグによる別名解決、json タグと併用したマッピング
    - jsontext.Value フィールド（json:",unknown" など）で未知ノードを生バイト保持
- 配列まわり
    - スカラー配列／オブジェクト配列のデコード
    - null、空配列、初期値リセット（既存スライス／ポインタスライスをゼロから上書き）
- ポインタ & Null
    - スカラーポインタ、構造体ポインタ、ポインタ配列
    - JSON null を Go の nil に落とす、非 null の場合は自動で確保
- インライン／明示的フラグメント
    - 構造体の匿名埋め込み＋graphql:"... on Type" で複数フラグメントを同一レスポンスに反映
    - ネストしたフラグメント階層、同じ JSON フィールドを複数フラグメントが共有するケース
- Union／インタフェース
    - __typename とインラインフラグメントで Union 型を表現
    - 各バリアント（例: ClosedEvent / ReopenedEvent）の専用フィールドを補完
- 未知フィールドの保持／エラー
    - json:",unknown" フィールドで丸ごと JSON を保持（例: jsontext.Value や map）
    - フォールバック先が無い場合は「構造体に該当フィールドが無い」旨のエラーを付与
- カスタムスカラー／列挙
    - UnmarshalGQL 実装を尊重（例: enum を文字列で受け取り変換）
    - encoding/json/encoding.TextUnmarshaler を実装した型も優先して利用
- ネストした選択
    - フラグメント内の配列 (commits.nodes など)
    - 深いネストと複合ルートフィールド (search -> nodes -> ...)