...
- フラグメント対応:
  - フラグメントスプレッドは `json:"-"` 付きの名前付きフィールド（型名と同名）として表現し、UnmarshalJSONFrom 内で同じ JSON データから直接デコードする。埋め込まないのは、複数フラグメントの同名フィールドが Go のフィールド昇格で曖昧になる（`t.X` がコンパイルエラー）のを避けるためで、アクセスは常に `t.<フラグメント名>.<フィールド>`
