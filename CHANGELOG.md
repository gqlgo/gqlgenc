...
- querygen がオペレーションごとのレスポンス型に型安全な `UnmarshalJSON` を生成します。一度 `map[string]jsontext.Value` にデコードして各フィールドを個別にアンマーシャルする実装で、フラグメントスプレッドは `json:"-"` 付きの埋め込み構造体として同じ JSON データから直接デコードし、インラインフラグメント（`... on Type`）は `__typename` の値による型判別でデコードします。デコードロジックがコンパイル時に確定するため、ランタイムの汎用デコーダが不要になりました
