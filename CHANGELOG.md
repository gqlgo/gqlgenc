...
- クエリのフラグメント定義やフィールド選択に `@goModel(model: "import/path.Type")` を付けると、レスポンス型を生成せず指定した既存 Go 型にバインドします。型を共有したり、生成型にできないメソッドを持たせたい場合に使えます。gqlgen の `@goModel`（スキーマ型のバインド）と同じディレクティブ・引数を、クエリ側のロケーション（フラグメント定義・フィールド）に拡張して再利用します。`@goModel` はクライアント側のコード生成専用ディレクティブで、サーバーへ送るクエリからは自動的に除去されます
